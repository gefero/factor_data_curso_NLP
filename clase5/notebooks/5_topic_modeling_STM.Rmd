...
stm_15 <- stm(documents = revistas_dfm,
     ...
     data = metadata,
     init.type = "Spectral")
write_rds(stm_15, '../models/stm_15_cont.rds')
toc()
...
```{r}
findThoughts(stm_15, texts=metadata %>% mutate(text = str_sub(text, 1, 200)) %>% select(text) %>% pull(), 
             n=10, 
             topics=9)
```

Al ver los primeros 5 documentos parece claro que se trata de tópico que habla de autos, sobre todo.
